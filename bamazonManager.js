...
var connection = mysql.createConnection({
    ...
    database: "BamazonMYSQL"
});

//Functions
function displayAll() {
    //show all ids, names, and products from database.
    connection.query('SELECT * FROM StoreProducts', function(error, response) {
...
function restockDatabase(id, quant) {
    //update the database
    connection.query('SELECT * FROM StoreProducts WHERE ItemID = ' + id, function(error, response) {
        if (error) { console.log(error) };
        connection.query('UPDATE StoreProducts SET StockQuantity = StockQuantity + ' + quant + ' WHERE ItemID = ' + id);
...
function buildNewItem(name,category,price,quantity){
	...
	connection.query('INSERT INTO StoreProducts (ProductName,DepartmentName,Price,StockQuantity) VALUES("' + name + '","' + category + '",' + price + ',' + quantity +  ')');
...
function removeFromDatabase(id){
	connection.query('DELETE FROM StoreProducts WHERE ItemID = ' + id);
	displayAll();
